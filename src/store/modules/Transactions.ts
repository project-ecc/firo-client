...
function txosFromTx(tx: Transaction): TXO[] {
    ...
    for (const txout of tx.outputs) {
        ...
        else if (tx.inputType == 'zerocoin') validAt = Infinity;
        else if (tx.inputType == 'sigma') validAt = tx.blockHeight + 6;
        else if (isPrivate && !tx.blockHeight) validAt = Infinity;
        else if (isPrivate) validAt = tx.blockHeight + 1;
        else if (tx.inputType == 'lelantus') validAt = 0;
        else if (tx.inputType == 'public') validAt = 0;
